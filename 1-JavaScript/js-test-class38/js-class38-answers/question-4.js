...
const recommendedNextVideo = (users = [], videos = [], userId = null) => {
  ...
  const favoritedVideoIds = users.find(
    (user) => user.id === userId
  ).favoritedVideoIds;

  favoritedVideoIds.forEach((videoId) => {
    const favoriteVideo = videos.find((video) => video.id === videoId);
    favoriteVideo.tags.forEach((favTagId) => {
      favoriteTagIds.push(favTagId);
    });
  });

  // create a list of videos that the user has not seen yet!
  const nonSeenVideos = videos.filter(
  ...
  nonSeenVideos.forEach((video) => {
    let count = 0;
    video.tags.forEach((tag) => {
      if (favoriteTagIds.includes(tag)) {
        count = count + 1;
      }
    });

    if (currentFavorite == null || count >= currentFavoriteCount) {
      currentFavorite = video;
